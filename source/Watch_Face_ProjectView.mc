...
class WeatherManager {
    ...
    private var _conditionCode    as Number  = 13; // CONDITION_UNKNOWN_PRECIPITATION as safe default
...
//
//  Ring layout:
//    Top-left     (180°→90°)   = Calories
//    Bottom-left  (270°→180°)  = Steps
...
class Watchface1View extends WatchUi.WatchFace {
    ...
    private function getBackgroundColor(h as Number) as Number {
        if (h >= 6 && h < 11) {
            return 0x1A1A2E;   // Morning – warm dark navy
        } else if (h >= 11 && h < 14) {
            return 0x16213E;   // Midday – deep steel blue
        } else if (h >= 14 && h < 18) {
            return 0x1B1B2F;   // Afternoon – warm dark slate
        }
        return 0x000000;       // Night – pure black (AMOLED friendly)
    }

    // Primary text (time, temp)
    private function getTimeColor(h as Number) as Number {
        if (h >= 6 && h < 11) {
            return 0xFFBE76;   // Morning – soft amber
        } else if (h >= 11 && h < 14) {
            return 0xFFFFFF;   // Midday – bright white
        } else if (h >= 14 && h < 18) {
            return 0xF6B93B;   // Afternoon – warm gold
        }
        return 0xA4B0BE;       // Night – cool silver-blue
    }

    // Secondary text (date, fallback weather)
    private function getDateColor(h as Number) as Number {
        if (h >= 6 && h < 11) {
            return 0xD4A76A;   // warm tan
        } else if (h >= 11 && h < 14) {
            return 0xC8D6E5;   // light blue-gray
        } else if (h >= 14 && h < 18) {
            return 0xD4A76A;   // warm tan
        }
        return 0x636E72;       // dim gray
    }

    // Divider line
    private function getDividerColor(h as Number) as Number {
        if (h >= 6 && h < 11) {
            return 0x8B7355;   // muted bronze
        } else if (h >= 11 && h < 14) {
            return 0x576574;   // steel blue
        } else if (h >= 14 && h < 18) {
            return 0x8B7355;   // muted bronze
        }
        return 0x2F3640;       // dark slate
    }
    ...
    function onUpdate(dc as Dc) as Void {
        ...
        var timeString = Lang.format("$1$:$2$", [hour.format("%02d"), minutes]);
        ...
        if (calPercent > 0) {
            ...
            dc.drawArc(centerX, centerY, ringRadius, Graphics.ARC_CLOCKWISE, 180, 180 - (calPercent * 90).toNumber());
        }

        // ── STEPS  (bottom-left quarter: 270° → 180°) ───────
        ...
        if (stepPercent > 0) {
            ...
            dc.drawArc(centerX, centerY, ringRadius, Graphics.ARC_CLOCKWISE, 270, 270 - (stepPercent * 90).toNumber());
        }
        ...
        if (_weatherMgr.isDataAvailable()) {
            ...
            if (cond == Weather.CONDITION_CLEAR ||
                cond == Weather.CONDITION_PARTLY_CLOUDY) {
                if (cond == Weather.CONDITION_CLEAR) {
            ...
            } else if (cond == Weather.CONDITION_FOG ||
                       cond == Weather.CONDITION_HAZY ||
                       cond == Weather.CONDITION_DUST ||
                       cond == Weather.CONDITION_SMOKE ||
                       cond == Weather.CONDITION_SANDSTORM) {
                weatherIcon = Rez.Drawables.ImgCloudy;
            } else {
                weatherIcon = Rez.Drawables.ImgPartlyCloudy;
            }
            ...
            var tempStr = _weatherMgr.getTemperature().format("%d") + _weatherMgr.getTemperatureUnit();

            dc.setColor(getTimeColor(hour), Graphics.COLOR_TRANSPARENT);
            dc.drawText(weatherX, weatherY + 44, Graphics.FONT_XTINY, tempStr, Graphics.TEXT_JUSTIFY_CENTER);
        } else {
            dc.setColor(getDateColor(hour), Graphics.COLOR_TRANSPARENT);
            dc.drawText(weatherX, weatherY + 10, Graphics.FONT_XTINY, "--", Graphics.TEXT_JUSTIFY_CENTER);
        }
    }
